...
Functions, as you may remember, are objects that store commands. They ease using instructions repeatedly with different inputs. The three basic parts of a function are name, code to implement, and arguments. To assemble these parts, we can use the `function()` function (yes, really) followed by a pair of curly brackets `{}`:
...
All of the objects that we create inside the function will disappear after it finishes running. We can only save the output of the function by assigning it to an object.
:::
