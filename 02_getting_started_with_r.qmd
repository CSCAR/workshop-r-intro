...
In simple terms, an object is a named piece of information. A single number, a word, a plot, and a set of instructions can all be objects if you name them. The name allows you to access the piece of information whenever you want. To create an object, you must first choose a name, then type an arrow `<-` (a "lower than" symbol `<` followed by a "minus" sign `-`), and then write the value that you want to save.
...
Be careful with names. If you try to reuse a name, **[R]{.sans-serif}** will overwrite the previous object without notifying you in any way.
...
The output shows that `round()` has two arguments: `x` (the number you want to round) and `digits`. It also shows that `digits` is already set to zero. This means that **[R]{.sans-serif}** will use `0` as the default value. Arguments with default values are optional because you can choose to modify them or not.

I encourage you to name all the arguments in the function, or at least the ones after the first argument. It is hard to remember all the arguments that can go into more complicated functions. And it is even harder to remember the correct order. Accidentally switching the order of the arguments can produce wrong results without you knowing. Naming the arguments will prevent errors and clarify the function's behavior.
