...
To extract elements based on their position we simply write the position inside the `[ ]`. Let's first recall the values of `my_vec` For example, 
...
We can extract multiple elements at the time by using a vector of indices inside the square brackets:

```{r}
my_vec[c(1, 3, 5)]
```

Or we can extract a range of values using the `:` notation. Remember that `:` helps us create a sequence of values. For example,
