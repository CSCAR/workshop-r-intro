...
Note that in **[R]{.sans-serif}**, the positional index starts at 1, so to call the first element of a vector we need to use `[1]`. In almost all other programming languages (like Python and C++), the positional index starts at 0.
...
I want to order the vector of countries, going from coldest to hottest. The first step to reorder the countries is to use `order()` to create a new variable called "temperatures_ordered".
...
This vector manipulations can do more than dazzle your friends. Imagine you have a dataset with two columns of data and you want to sort each column. If you just use `sort()` on each column separately, the values of each column will become uncoupled from each other. By using the `order()` on one column, a vector of positional indices is created of the values of the column in ascending order. This vector can be used on the second column, as the index of elements which will return a vector of values based on the first column.

## Working with scripts

Until now, the console satisfied all our coding needs. But if we wanted to reuse a command from before, we would have to rewrite it from scratch; or we could look through our command history (in the "Environment-History-Connections" window)---a tedious task. Besides, once we close this session in RStudio, all of our work will be gone, like tears in the rain. It would be much better if we wrote our code in a place where we can easily review and modify it.
...
We can run a line of code using the `Run` button. R will run whichever line of code your cursor is on. We can also run all of the code in the script by clicking the `Source` button that's next to `Run`. I find that clicking buttons is slow, so I use a keyboard shortcut for running a line of code: `Control + Return` in Windows and Linux; `Command + Return` in Mac.

[![Buttons for running code](./images/run_code_buttons_edited.jpg)](./images/run_code_buttons_edited.jpg)

## Writing your own functions

Functions, as you may remember, are objects that store commands. They are helpful when we want to use instructions repeatedly with different inputs. 

## Exercise

Let's try to write a function using all of the basic features of **[R]{.sans-serif}** that you just learned. 