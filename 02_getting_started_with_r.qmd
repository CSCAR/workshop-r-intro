...
Later on we will learn how to check if two numbers are "close enough" to each other.
...
Two useful functions to extract values of a vector are `head()`, which extracts the first few values of a vector

```{r}
head(my_vec, n = 3)
```

and `tail()`, which gives us the last few values of a vector
