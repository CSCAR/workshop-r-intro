...
```
Treating `NA` as a type of logical value allows **R** to handle missing data in
...
typeof(my_character)
```
It is easy to confuse objects with character strings because both look like
chunks of text in the code. For example, `the_thing` is the name of an object
named *the_thing* that can contain any type of data. Conversely, `"the_thing"`
is a character string, i.e., a piece of data that we can assign to any name. If
we forget the quotation marks when writing a name, **R** will look for an object
that likely doesn't exist, so we will likely get an error.
```{r}
#| error: true
noquotes
```
...
```
Still, we can differentiate strings from actual numbers because **R** always
shows strings surrounded by quotation marks and in different colors from other
data types.
...
A special type of character string is a *factor.* Factors are **R**'s way of
storing categorical information, like color or level of agreement. They can only
have certain values (e.g., "red" or "green") called "levels", which may have
their own order (e.g., "agree", "neutral", "disagree"). To create a factor, we
can start with a vector of strings and use function `as.factor()` to convert it
to a factor.
```{r create factor}
...
Atomic vectors can have almost^[The maximum length (number of elements) of a vector is 2^31 - 1 ~ 2*10^9. See [here](https://astrostatistics.psu.edu/su07/R/html/base/html/Memory-limits.html)
for more information.] as many elements as you want (including zero).
...
This requirement seems rigid, but it allows the computer to store large sets of
numbers in a simple and efficient way; and it accelerates computations because
**R** knows that it can manipulate all values in the object in the same way. 

However, we often need to store different types of data in a single
place---maybe because all of that data belongs to the same underlying subject of
...
**R** can keep all these information in a single place.
...
The `str()` function gives us the data frame dimensions and also reminds us that
`aliens_df` is a data.frame type object. `str()` also lists all of the variables
(columns) contained in the data frame, tells us what type of data the variables
contain, and prints the first five values.
